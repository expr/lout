...
internals.getParamsData = function (param, name) {
    ...
    if (Array.isArray(param)) {
        ...
        type = 'alternatives';
    } else if (param.valids && param.valids.some(function(v) { return typeof v === 'function' && v.path })) {
    ...
    if (data.type === 'object' && param.children) {
        ...
        param.rules.forEach(function (rule) {
            ...
            data.rules[capitalizedName] = !Array.isArray(rule.arg) ? rule.arg : rule.arg.map(function (type) {
                ...
                return internals.describe(type);
            });
        });
...
internals.getExistsValues = function (exists) {
    ...
    var values = exists.filter(function (value) {
        ...
        if (typeof value === 'function' && 'key' in value) { // Transform references into their key pattern
